...
interface FontPreviewLoaderProps {
...
export function FontPreviewLoader({ fontFamily }: FontPreviewLoaderProps) {
  useEffect(() => {
    if (!fontFamily || !GOOGLE_FONTS[fontFamily]) return;

    // Check if font is already loaded
    const fontId = `font-preview-${fontFamily.replace(/\s+/g, "-").toLowerCase()}`;
